...
jsPsych.plugins['canvas-segmentation'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      scale: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Canvas scale',
        default: 1.0,
        description: 'Scale the canvas by this factor.'
      },
      choices: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if(trial.canvasId !== false) {
      ...
      canvas = '<canvas id="'+trial.canvasId+'" height="'+trial.scale*trial.canvasHeight+
        '" width="'+trial.scale*trial.canvasWidth+'"></canvas>';
    }
    ...
    html += '<div id="jspsych-canvas-segmentation">'+canvas+'</div>';
    ...
    var after_response = function(info) {
      ...
      display_element.querySelector('#jspsych-canvas-segmentation').className += ' responded';
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-canvas-segmentation').style.visibility = 'hidden';
